...
class JACMeanAggregatorReplicableFeature(ReplicableFeature, AggregatorFeatureMixin):
    def __init__(self, value: Tuple[jnp.array, int, FrozenDict] = None, tensor: "jnp.array" = None, *args, **kwargs):
        ...
        stored_value = [tensor, 0, None]  # Represents (tensor, number_of_elements, jackobian)
        super(JACMeanAggregatorReplicableFeature, self).__init__(*args, value=stored_value, **kwargs)

    @rpc
    def reduce(self, new_element: "Tuple[jnp.array, FrozenDict]", count=1):
        """ Supports Bulk Reduce with 1 sync. step """
        self._value[0] = ((self._value[0] * self._value[1]) + new_element[0]) / (self._value[1] + count)
        if self._value[2] is None:
            # No Jackobians yet
            self._value[2] = new_element[1]
        else:

        self._value[1] += count
        return True

    def bulk_reduce(self, *new_elements: Tuple[Tuple[jnp.array, FrozenDict]]):
    ...
    @rpc
    def replace(self, new_tensor: Tuple[jnp.array, FrozenDict], old_tensor: Tuple[jnp.array, FrozenDict]):
        if self.value[1] == 0:
    ...
    def bulk_replace(self, *elements: List[Tuple[jnp.array, FrozenDict]]):
        if len(elements) == 0:
            return
