""" Simply classes for easier dataset handling """
import os
import pandas as pd
import numpy as np
...
class EdgeListGraph:
    def __init__(self, folder_name, file_name, extension, sep, usecols=None, transform=None):
        self.folder_name = folder_name
        self.file_name = file_name
        self.extension = extension
        self.sep = sep
        self.dataset = pd.read_csv(os.path.join(os.environ["DATASET_DIR"], folder_name, f"{file_name}.{extension}"),
                                   header=None, sep=sep, usecols=usecols)
    ...
    def save_tsv(self):
        self.dataset.to_csv(os.path.join(os.environ["DATASET_DIR"], self.folder_name, f"{self.file_name}.tsv"),
                            sep='\t', index=False, header=False)


class DGraphFin(EdgeListGraph):
    def __init__(self, folder_name, file_name):
        ...
        self.dataset = pd.DataFrame(
            np.hstack((self.npzarr['edge_index'][order], self.npzarr['edge_timestamp'][order].reshape((1, -1)).T)))
        nodes = pd.concat((self.dataset[0], self.dataset[1])).unique()
        self.nodes_2_index = dict(zip(nodes, range(len(nodes))))
        self.index_2_nodes = {v: k for k, v in self.nodes_2_index.items()}

    def save_feature_and_labels(self):
        np.save(os.path.join(os.environ["DATASET_DIR"], self.folder_name, "node_features"),
                self.npzarr['x'].astype("float32"))
        np.save(os.path.join(os.environ["DATASET_DIR"], self.folder_name, "node_labels"),
                self.npzarr['y'].astype("int8"))
