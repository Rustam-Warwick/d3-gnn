...
import operators.iterations.MpscQueue;
...
public class BufferPerformanceTests{
    ...
    public long testMpscQueue(){
        long ms  = System.currentTimeMillis();
        MpscQueue<Integer> a = new MpscQueue(32 * 1024, Locks.spinLock());
        ArrayList<Thread> producers = new ArrayList<>();
        Phaser phaser = new Phaser();
        phaser.register();
        for (int i = 0; i < 1; i++) {
        ...
        producers.forEach(Thread::start);
        while(phaser.getArrivedParties() < 1){
            for (Integer integer : a.drainAll()) {
                if(integer % 100000 ==0 ) System.out.println(integer);
            }
        }
        return System.currentTimeMillis() - ms;
    ...
    public void compareQueues(){
        ...
        long mpscQueue = testMpscQueue();
        long spscQueue = testSPSCQueue();
        System.out.format("Transfer: %s | Linked: %s | MPSC: %s | SPSC: %s\n", linkedTransferQueue, linkedQueue, mpscQueue, spscQueue);
    }
