...
public class StreamingHGNNEmbeddingLayer extends Plugin {
        ...
        if(element.elementType() == ElementType.VERTEX){
            ...
            for (Vertex vertex : edge.getVertices()) {
                ...
                if(messageReady(vertex)){
                    ...
                    new RemoteInvoke()
                            .toElement(Feature.encodeAttachedFeatureId("agg", edge.getId()), ElementType.FEATURE)
            ...
            if(feature.getName().equals("feature") && feature.attachedTo != null && feature.attachedTo.f0 == ElementType.VERTEX){
                ...
                for (Vertex vertex : edge.getVertices()) {
                    new RemoteInvoke()
                            .toElement(Feature.encodeAttachedFeatureId("agg", vertex.getId()), ElementType.FEATURE)
                            .where(MessageDirection.ITERATE)
                            .method("reduce")
                            .hasUpdate()
                ...
                for (String vertexId : newEdge.vertexIds) {
                    if(!oldEdge.vertexIds.contains(vertexId)){
                        ...
                        if(messageReady(v)){
                            ...
                            new RemoteInvoke()
                                    .toElement(Feature.encodeAttachedFeatureId("agg", newEdge.getId()), ElementType.FEATURE)
                                    .where(MessageDirection.ITERATE)
                                    .method("reduce")
                                    .hasUpdate()
    ...
    public void reduceHyperEdges(Vertex v){
        ...
        for (HEdge hyperEdge : storage.getHyperEdges(v)) {
            ...
            new RemoteInvoke()
                    .toElement(Feature.encodeAttachedFeatureId("agg", hyperEdge.getId()), ElementType.FEATURE)
                    .where(MessageDirection.ITERATE)
                    .method("reduce")
                    .hasUpdate()
