...
public class WindowedGNNEmbeddingLayer extends Plugin {
    ...
    public void initVertex(Vertex element) {
        if (element.state() == ReplicaState.MASTER) {
            NDArray aggStart = NDHelper.globalNDManager.zeros(modelServer.getModel().describeInput().get(0).getValue(), modelServer.getModel().getDataType());
            element.setFeature("agg", new MeanAggregator(aggStart, true));

            if (!externalFeatures && storage.layerFunction.isFirst()) {
                NDArray embeddingRandom = NDHelper.globalNDManager.ones(modelServer.getModel().describeInput().get(0).getValue(), modelServer.getModel().getDataType()); // Initialize to random value
