...
public class MixedGNNEmbeddingLayerTraining extends Plugin {
    ...
    public void trainFirstPartStart(){
        ...
        if(isLastReplica()){
            ...
            storage.layerFunction.sideBroadcastMessage(new GraphOp(Op.RMI, null, iterationSynchronize, null, MessageCommunication.BROADCAST), BaseWrapperOperator.ITERATE_OUTPUT_TAG);
        }
    }
    ...
    public void trainSecondPartStart(){
        ...
        if(collectedAggregators != null && !collectedAggregators.getValue().isEmpty()){
            ...
            HashMap<BaseAggregator<?>, List<Vertex>> reverseEdgeList = new HashMap<>();
            List<Vertex> srcVertices = new ArrayList<>();
            for(Map.Entry<BaseAggregator<?>, NDArray> entry: collectedAggregators.getValue().entrySet()) {
                ...
                for (Edge inEdge : inEdges) {
                    ...
                    if(!srcVertices.contains(inEdge.src)){
                        ...
                        inEdge.src.setStorage(null); // Remove from storage
            ...
            // 3. Cleanup
            srcFeatures.forEach(item->item.get(0).setRequiresGradient(false));
